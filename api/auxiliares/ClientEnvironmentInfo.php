...
final class ClientEnvironmentInfo
{
    ...
    public function __construct(string $geoDbPath = '')
    {
        ...
        if (empty($geoDbPath)) {
            $geoDbPath = $_SERVER['DOCUMENT_ROOT'] . 'api/auxiliares/geolite.mmdb'; // AJUSTA si es necesario
        }
    ...
    {
        $os = strtolower($this->getClientOs());
        if (str_contains($os, 'android') || str_contains($os, 'iphone') || str_contains($os, 'ipad')) {
            return 'mobile';
        }
        return 'desktop';
    }
    ...
    public function getAllClientMetadata(): array
    {
        require_once  __DIR__ . '/session_timezone_helper.php';
        $geo = $this->getGeoInfo();
        return [
            'ip' => $this->getClientIp(),
            'pais' => $geo['client_country'],
            'region' => $geo['client_region'],
            'ciudad' => $geo['client_city'],
            'codigo_postal' => $geo['client_zipcode'],
            'coordenadas' => $geo['client_coordinates'],
            'zona_horaria' => $this->getTimezoneRegion(),
            'geo_ip_timestamp' => getNowInUserLocalTime(
                $geo['client_country'],
                $geo['client_region'],
                $geo['client_city']
            ),
            'dominio' => $this->getDomainName(),
            'hostname' => $this->getClientHostname(),
            'sistema_operativo' => $this->getClientOs(),
            'navegador' => $this->getClientBrowser(),
